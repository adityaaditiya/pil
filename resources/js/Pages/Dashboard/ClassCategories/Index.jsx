...
function CategoryCard({ classCategory }) {
    return (
        <div className="group bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden hover:shadow-lg hover:border-slate-300 dark:hover:border-slate-700 transition-all duration-200">
            ...
            <div className="relative aspect-[3/2] bg-slate-100 dark:bg-slate-800 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/40 transition-all flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                    <Link
                        href={route("class-categories.edit", classCategory.id)}
                    ...
                    <Button
                        ...
                        url={route("class-categories.destroy", classCategory.id)}
                    />
                </div>
            </div>
...
export default function Index({ classCategories }) {
        ...
        <>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            {classCategories.total || classCategories.data?.length || 0}{" "}
                            kategori kelas terdaftar
                        </p>
                    </div>
                    <Button
                        type={"link"}
                        ...
                        label={"Tambah Kategori Kelas"}
                        href={route("class-categories.create")}
                    />
                </div>
            </div>
            ...
            <div className="mb-4 flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3">
                <div className="w-full sm:w-80">
                    <Search
                        url={route("class-categories.index")}
                        placeholder="Cari kategori kelas..."
                    />
                </div>
            ...
            {/* Content */}
            {classCategories.data.length > 0 ? (
                viewMode === "grid" ? (
                    /* Grid View */
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                        {classCategories.data.map((classCategory) => (
                            <CategoryCard
                    ...
                    /* List View */
                    <Table.Card title={"Data Kategori Kelas"}>
                        <Table>
                            <Table.Thead>
                                <tr>
                            ...
                            <Table.Tbody>
                                {classCategories.data.map((classCategory, i) => (
                                    <tr
                                    ...
                                    >
                                        <Table.Td className="text-center">
                                            {++i +
                                                (classCategories.current_page - 1) *
                                                    classCategories.per_page}
                                        </Table.Td>
                                        <Table.Td>
                ...
                <div className="flex flex-col items-center justify-center py-16 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                        Tambahkan kategori kelas pertama Anda.
                    </p>
                    <Button
                        type={"link"}
            ...
            {classCategories.last_page !== 1 && (
                <Pagination links={classCategories.links} />
            )}
        </>
    );
}
