...
export default function Edit({ plan, classes }) {
    const { data, setData, put, processing, errors } = useForm({ ...plan, activation_setting: plan.activation_setting || "on_first_credit_use", class_rules: plan.class_rules.map((item) => ({ pilates_class_id: item.pilates_class_id, credit_cost: item.credit_cost })) });
