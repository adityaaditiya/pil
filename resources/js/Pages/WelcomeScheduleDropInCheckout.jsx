...
export default function WelcomeScheduleDropInCheckout({
    ...
    const cancelTransaction = () => {
        router.delete(
            route("welcome.schedule-payment.cancel-transaction", booking.id),
        );
    };
                ...
                <div className="mx-auto max-w-3xl rounded-3xl border border-primary-100 bg-white p-6 shadow-sm md:p-8">
                    ...
                    {!booking?.payment_proof_image && (
                        <p className="mt-2 rounded-xl bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-700">
                            Selesaikan transaksi dalam {minutes}:{seconds}.
                            Otomatis batal jika transaksi tidak diselesaikan
                        </p>
                    )}
                    {selectedGateway?.value === "qris" && (
                        <div className="mt-6 rounded-2xl border border-slate-200 p-4">
                            ...
                            <p className="mt-3 text-sm text-slate-700">
                                Nama Merchant:{" "}
                                <span className="font-semibold">
                                    {paymentInstructions?.qris_full_name || "-"}
                                </span>
                            </p>
                            <p className="mt-3 text-sm text-slate-700">
                                *Pastikan QRIS sesuai dengan yang tertera pada
                                Nama Merchant
                            </p>
                        </div>
                    )}
                    ...
                    {secondsLeft === 0 && (
                        <div className="mt-6 rounded-2xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">
                            Waktu pembayaran habis. Silakan ulangi proses
                            pembayaran.
                        </div>
                    )}
